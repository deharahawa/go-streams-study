...
func (c *userServiceClient) AddUserStreamBidi(ctx context.Context, opts ...grpc.CallOption) (UserService_AddUserStreamBidiClient, error) {
	stream, err := c.cc.NewStream(ctx, &UserService_ServiceDesc.Streams[2], "/pb.UserService/AddUserStreamBidi", opts...)
	if err != nil {
		return nil, err
	}
	x := &userServiceAddUserStreamBidiClient{stream}
	return x, nil
}

type UserService_AddUserStreamBidiClient interface {
	Send(*User) error
	Recv() (*UserResultStream, error)
	grpc.ClientStream
}

type userServiceAddUserStreamBidiClient struct {
	grpc.ClientStream
}

func (x *userServiceAddUserStreamBidiClient) Send(m *User) error {
	return x.ClientStream.SendMsg(m)
}

func (x *userServiceAddUserStreamBidiClient) Recv() (*UserResultStream, error) {
	m := new(UserResultStream)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type UserServiceServer interface {
	...
	AddUserStreamBidi(UserService_AddUserStreamBidiServer) error
...
func (UnimplementedUserServiceServer) AddUserStreamBidi(UserService_AddUserStreamBidiServer) error {
	return status.Errorf(codes.Unimplemented, "method AddUserStreamBidi not implemented")
}
...
func _UserService_AddUserStreamBidi_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(UserServiceServer).AddUserStreamBidi(&userServiceAddUserStreamBidiServer{stream})
}

type UserService_AddUserStreamBidiServer interface {
	Send(*UserResultStream) error
	Recv() (*User, error)
	grpc.ServerStream
}

type userServiceAddUserStreamBidiServer struct {
	grpc.ServerStream
}

func (x *userServiceAddUserStreamBidiServer) Send(m *UserResultStream) error {
	return x.ServerStream.SendMsg(m)
}

func (x *userServiceAddUserStreamBidiServer) Recv() (*User, error) {
	m := new(User)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var UserService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "AddUserStreamBidi",
			Handler:       _UserService_AddUserStreamBidi_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
